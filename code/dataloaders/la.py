...
from albumentations import (Blur,Flip,ShiftScaleRotate,GridDistortion,ElasticTransform,HorizontalFlip,CenterCrop,
                            ...
                            ,ChannelDropout,ISONoise,VerticalFlip,RandomGamma,RandomRotate90,OpticalDistortion,
                            RandomBrightness,RandomContrast,MedianBlur,MotionBlur,GaussianBlur,GaussNoise)
...
class LAHeart(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        h5f = h5py.File(self._base_dir+'/LA/data/'+image_name+"/mri_norm2.h5", 'r')
        image = h5f['image'][:]
        label = h5f['label'][:]
        ...
        if self.sp_transform:
            sample1 = sample
            sample2 = RandAugment(sample,n=3)
        if self.common_transform:
            sample1 = self.common_transform(sample1)
            sample2 = self.common_transform(sample2)
...
def my_augment_pool():
    augs = [
        RandomBrightness(limit=0.1, always_apply=False, p=0.5),
        RandomContrast(limit=0.1, always_apply=False, p=0.5),
...
def RandAugment(sample, augs=augs, n = 2, patch_size = (112, 112, 80)):
    ...
    del sample
    data_structure = {'image': new_sample['image']}
    aug_data = aug_strongly(**data_structure, force_apply=True, targets={})
    new_image = aug_data['image']
    new_sample = {'image': new_image, 'label': new_sample['label']}
    return new_sample
...
class RandomContrast(object):
    ...
    def __init__(self, random_state=np.random.RandomState(1234), alpha=(0.5, 1.5), mean=0.0, execution_probability=0.1, **kwargs):
